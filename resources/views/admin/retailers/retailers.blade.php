                ...
                <div class="small-6 columns no-padding">
                    ...
                    <div class="small-4 columns no-padding text-right">
                        <p class="font-11 showing-result">
                            ...
                            ?>
                           Showing {{$startFrom+1}} - {{$end}}						                                  of {{$retailers->total()}}</p>
                    </div>
                </div>
            </form>
        </div>
        ...
        <div class="content-box-content">
            <table width="100%" border="0" cellpadding="0" cellspacing="0">
                ...
                @foreach( $retailers as $retailer )
                    <tr>
                        <td><Input type = 'checkbox' class = 'id_of_retailers'  name = 'ids[]' value = {{$retailer->retailer_id}}></td>
                        <td>{{$retailer->retailer_id}}</td>
                        <td>{{$retailer->title}}</td>
                        <td>{{$retailer->Affnetwork->network_name or '---'}}</td>
                        <td>{{$retailer->cashback}}</td>
                        <td></td>
                        <td>{{$retailer->visits}}</td>
...
<!--                        --><?php //$d = new DateTime($retailer->added); ?>
                        <td>{{ date('d M Y', strtotime($retailer->added)) }}</td>
                        <td>
                            <a href="view/retailers/{{$retailer->retailer_id}}" class = 'black-color font-13 action tip-top' title ='view'>{!! '<i class ="fa fa-search"></i>' !!}</a>
                            <a href="edit/retailers?id={{$retailer->retailer_id}}" class = "black-color font-13 action tip-top" title = 'edit'>{!! '<i class="fa fa-pencil-square-o"></i>' !!}</a>

                            <form style="display:none;" name="post_tid{{$retailer->retailer_id}}" method="post" action="del/retailer"><input type='hidden' name='id' value='1' /><input type='hidden' name='action' value='deleteRetailer' /></form><a href='javascript:void(0);' onclick="if(confirm('Are you sure you want to delete this retailer?')){document.post_tid{{$retailer->retailer_id}}.submit();}else{return false;}" class='black-color font-13 action tip-top'  data-tooltip-title='Delete' ><i class="fa fa-trash-o"></i></a>
                        </td>
                    </tr>
                @endforeach
                </tbody>
